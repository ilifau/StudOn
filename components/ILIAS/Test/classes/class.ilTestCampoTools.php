<?php
// fau: campoGrades - new class ilTestCampoTools.

/* Copyright (c) 1998-2009 ILIAS open source, Extended GPL, see docs/LICENSE */

/**
* Tools for campo export of test results.
*/
class ilTestCampoTools
{
    ...
    /**
     * init the campo options for this test
     */
    public function initOptions()
    {
        ...
        $this->options['matriculations'] = '';
        $this->options['other_fields'] = '';		// firstname,lastname,datemstarttime
    }
    
    
    /**
     * read the campo options for this test from the database
     */
    public function readOptions()
    {
        ...
        $query = 'SELECT * FROM tst_campo_options WHERE obj_id='
    ...
    /**
     * write the campo options for this test to the datebase
     */
    public function writeOptions()
    {
        global $ilDB;
        
        $query = 'DELETE FROM tst_campo_options WHERE obj_id='
            . $ilDB->quote($this->test_obj->getId(), 'integer');
    ...
    public function checkRegistrationsFile($a_file)
    {
        ...
        if (!isset($matcol)) {
            return $lng->txt("ass_campo_matcol_not_found");
        }
     ...
     */
    public function createExportFiles(): array
    {
        global $lng;
        ...
        //$matriculations = explode('\r\n', $this->getOption('matriculations'));
        ...
        foreach ($matriculations as $matriculation)
        {
            ...
            foreach ($data->getParticipants() as $active_id => $participant)
            {
              ...
              //          if ($this->getOption('mark_field') == 'short') {
                            $row[] = $participant->getMark()->getShortName();
              //          } else {
                            // Noten für campo sollen als Zahlen ausgegeben werden, also die Kurzbezeichnung
                            $row[] = $participant->getMark()->getOfficialName();
              //            $row[] = $participant->getMark();
              //          }
              /*      } else {*/
                        if ($participant->getMark()->getPassed()) {
                            $row[] = '+';
                        } else {
                            $row[] = '-';
                        }
                   /* }*/
                    ...
                    if (in_array('date', $other_fields)) {
                        $row[] = date('d.m.Y', $start->getTimestamp());
                    }
                    if (in_array('starttime', $other_fields)) {
                        $row[] = date('H:i', $start->getTimestamp());
                    }
                }
       ...
       // ksort($rows);
        $file_name = "";
        $file_name = $this->writeExportFileCVS($header, $rows);
        ...
        if ($this->check_registrations) {
            ...
            if ($mess) {
                $mess = sprintf($lng->txt('ass_campo_missing_participants'), $mess);
            }
     ...
     */
    public function writeExportFileCVS($a_header = array(), $a_rows = array()): string
    {
        ...
        fwrite($file,  mb_convert_encoding(implode(';', $a_header) . "\r\n", 'ISO-8859-1', 'UTF-8'));
