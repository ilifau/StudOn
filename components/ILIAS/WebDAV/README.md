...
version 5.4, the sabreDAV library from sabre-io is used to handle the
HTTP-Requests.
...
To upload files, the WebDAV Client *Finder* on Mac uses chunked transfer
encoding.
...
Sabre (the creator of the used WebDAV library) has following things to say to
this issue:
...
> only server I've seen so far that handles this reasonably well is Apache +
> mod_php.
> Nginx and Lighttpd respond with 411 Length Required, which is completely
> ignored
