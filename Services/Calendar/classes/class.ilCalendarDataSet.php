...
// fau: fixPdImportXss - apply stripSlashes
						$category->setTitle(ilUtil::stripSlashes($a_rec["Title"]));
						$category->setColor(ilUtil::stripSlashes($a_rec["Color"]));
// fau.
...
// fau: fixPdImportXss - apply stripSlashes
					$entry->setTitle(ilUtil::stripSlashes($a_rec["Title"]));
					$entry->setSubtitle(ilUtil::stripSlashes($a_rec["Subtitle"]));
					$entry->setDescription(ilUtil::stripSlashes($a_rec["Description"]));
					$entry->setLocation(ilUtil::stripSlashes($a_rec["Location"]));
					$entry->setFullday(ilUtil::stripSlashes($a_rec["Fullday"]));
					if ($a_rec["Starta"] != "")
					{
...
// fau: fixPdImportXss - apply stripSlashes
					$rec->setRecurrence(ilUtil::stripSlashes($a_rec["CalRecurrence"]));
					$rec->setFrequenceType(ilUtil::stripSlashes($a_rec["FreqType"]));
					if ($a_rec["FreqUntilDate"] != "")
					{
						$rec->setFrequenceUntilDate(new ilDateTime(ilUtil::stripSlashes($a_rec["FreqUntilDate"], IL_CAL_DATETIME)));
					}
					$rec->setFrequenceUntilCount(ilUtil::stripSlashes($a_rec["FreqUntilCount"]));
					$rec->setInterval(ilUtil::stripSlashes($a_rec["Interval"]));
					$rec->setBYDAY(ilUtil::stripSlashes($a_rec["Byday"]));
					$rec->setBYWEEKNO(ilUtil::stripSlashes($a_rec["Byweekno"]));
					$rec->setBYMONTH(ilUtil::stripSlashes($a_rec["Bymonth"]));
					$rec->setBYMONTHDAY(ilUtil::stripSlashes($a_rec["Bymonthday"]));
					$rec->setBYYEARDAY(ilUtil::stripSlashes($a_rec["Byyearday"]));
					$rec->setBYSETPOS(ilUtil::stripSlashes($a_rec["Bysetpos"]));
					$rec->setWeekstart(ilUtil::stripSlashes($a_rec["Weekstart"]));
// fau.
					$rec->save();
