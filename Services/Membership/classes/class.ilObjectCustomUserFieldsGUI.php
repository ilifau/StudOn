...
class ilObjectCustomUserFieldsGUI
{
    ...
    protected function initFieldForm(int $a_mode): ilPropertyFormGUI
    {
        ...
        $ty_cb = new ilRadioOption($this->lng->txt('ps_type_checkbox'), (string) ilCourseDefinedFieldDefinition::IL_CDF_TYPE_CHECKBOX);
        $ty->addOption($ty_cb);
        // fau.
        //		Select Type
        ...
        //	Email Type
        $ty_em = new ilRadioOption($this->lng->txt('ps_type_email_long'), (string) ilCourseDefinedFieldDefinition::IL_CDF_TYPE_EMAIL);
        $ty->addOption($ty_em);
        ...
        if (!empty($poss_parents)) {
            ...
            foreach ($poss_parents as $parent_field) {
                $pf = new ilRadioOption($parent_field->getName(), (string) $parent_field->getId());
