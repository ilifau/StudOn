...
- Nr of LoCs changed in last 365 days: `git diff --shortstat HEAD $(git log --since=365.days --oneline components/ILIAS/WebDAV/ | tail -1 | cut -f 1 -d " ")^ <Path to Component>`
- Nr of Commits in last 365 days: `git log --since=365.days --oneline <Path to Component> | wc -l`
- To check the general code quality: `./scripts/PHPStan/run_checks.sh <Path to Component>`.
